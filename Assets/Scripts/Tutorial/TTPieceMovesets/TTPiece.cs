...
public class TTPiece : MonoBehaviour
{
    ...
    public TTPieceType originalType;
    public TTPieceType type;
    public bool isNavy;
        ...
        for (int x_change = -1; x_change < 2; x_change++)
        {
            for (int y_change = -1; y_change < 2; y_change++)
            {
                if (IsSquareOnBoard(currentX + x_change, currentY + y_change))
                {
                    ...
                    else if (isNavy != possibleSquare.currentPiece.isNavy && possibleSquare.currentPiece.type != TTPieceType.LandMine)
